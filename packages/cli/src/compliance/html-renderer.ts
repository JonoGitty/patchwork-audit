...
function statusBadge(status: ControlStatus): string {
	return `<span class="badge" style="background:${STATUS_COLORS[status]}">${STATUS_LABELS[status]}</span>`;
}

function riskBadge(level: string): string {
	return `<span class="badge" style="background:${RISK_COLORS[level] || "#444"}">${level.toUpperCase()}</span>`;
}
...
<section class="report-footer">
	...
	<table>
		<tbody>
			...
			<tr><td>Policy</td><td>${data.policySource}</td></tr>
		</tbody>
	</table>
